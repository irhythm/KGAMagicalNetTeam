...
public class GarrisonChaseState : AIStateBase
{
    ...
    public GarrisonChaseState(BaseAI ai, StateMachine machine) : base(ai, machine, BaseAI.AIStateID.Chase)
    {
        garrison = ai as GarrisonGuardAI;
        sqrAttackRange = garrison.attackRange * garrison.attackRange;
    }
    ...
    public override void Enter()
    {
        ...
        if (garrison.targetPlayer != null)
        {
            ...
            if (garrison.Agent.isOnNavMesh)
            {
                ...
                if (dir != Vector3.zero)
                {
                    garrison.transform.rotation = Quaternion.LookRotation(dir);
                }
    ...
    public override void Execute()
    {
        ...
        //사거리체크
        Vector3 toTarget = garrison.targetPlayer.position - garrison.transform.position;
        ...
        {
            //어택
            stateMachine.ChangeState(new GuardAttackState(garrison, stateMachine));
            return;
        }
        ...
        if (pathTimer > 0.25f)
        {
            ...
            if (Vector3.SqrMagnitude(garrison.targetPlayer.position - lastTargetPos) > 0.25f)
            {
                if (garrison.Agent.isOnNavMesh)
                {
                    garrison.Agent.SetDestination(garrison.targetPlayer.position);
